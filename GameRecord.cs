class GamesRecord 
{
  ...
  public void DisplayGamesHistory () {
    ...
    System.Console.WriteLine ("Last games history:");
    for (int i = 0; i < gamesRecordCurrentSize; i++){
      System.Console.WriteLine ("Game #{0}:\t{1}\t-\t{2},\t{3}", i+1, gamesRecord[displayRecordIndex,0], gamesRecord[displayRecordIndex,1], gamesRecord[displayRecordIndex,2]);
